...
library(broom.mixed) # for tidying mulitilevel models
```
...
personality_items = personality_items %>%
  # apply to all varibles except proid
...
```{r 1-cleaning-24, fig.cap = "Maximum number of same consecutive responses in personality blocks."}
#visualize
runs_data %>%
...
in_average = data_t1 %>%
  ...
  mutate(
    remove = case_when(
      ...
      TRUE ~ "Keep"))
```
```{r 1-cleaning-27, fig.cap = "Average response to inattention check items"}
in_average %>%
...
```{r 1-cleaning-33, fig.cap = "Distribution of average time to respond to personality items."}
timing_data %>%
...
```{r 1-cleaning-41, echo = F}
# there was an error in the survey used to collect pilot data: participants were supposed to see
# either "tend to be outgoing" or "I tend to be outgoing" but all participants saw both conditions. 
# this creates errors in later code.
# to fix this issue when coding, we chose to remove the responses to "I tend to be outgoing", which
# means outgoing will not be presented in test-retest analyses
data_2 = data_2 %>% 
...
```{r 1-cleaning-55, fig.cap = "Maximum number of same consecutive responses in personality block 3."}
#visualize
runs_data_2 %>%
...
in_average = data_2 %>%
  ...
  mutate(
    remove = case_when(
      ...
      TRUE ~ "Keep"))
```
```{r 1-cleaning-59, fig.cap = "Average response to inattention check items"}
in_average %>%
  ggplot(aes(x = avg, fill = remove)) +
  geom_histogram(bins = 20, color = "white") +
  geom_vline(aes(xintercept = 4)) +
...
```{r 1-cleaning-64, fig.cap = "Distribution of average time to respond to personality items in Block 3."}
timing_data_2 %>%
  ggplot(aes(x = m_time, fill = remove)) +
  geom_histogram(color = "white") +
  labs(x = "Average response time (seconds)", y = "Number of participants") +
...
```{r 1-cleaning-76, fig.cap = "Correct responses on the memory task"}
data %>%
  ggplot(aes(x = memory)) +
...
```{r 1-cleaning-79, fig.cap = "Distribution of delayed memory scores"}
data %>%
...
```{r 1-cleaning-80, fig.cap = "Relationship between immediate and delayed recall"}
data %>%
...
```{r 1-cleaning-82, fig.cap = "Distribution of delayed memory scores"}
data %>%
...
```{r 1-cleaning-83, fig.cap = "Relationship between immediate and delayed recall"}
data %>%
...
Longer labels were provided to participants ifor clarity. However, we will use shorter labels in our analyses and figures.
...
```{r 1-cleaning-95, fig.cap= "Distribution of seconds, raw and transformed."}
items_df %>% 
