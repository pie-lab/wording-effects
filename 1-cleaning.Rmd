...
```{r}
names(data) = str_replace(names(data), "broad_mind", "broadmind")
names(data) = str_replace(names(data), "self_disciplind", "selfdisciplined")
```
...
```{r 1-cleaning-12, ref.label='rm_proid'}
```
...
```{r 1-cleaning-18, fig.cap = "Maximum number of same consecutive responses in personality blocks."}
#visualize
runs_data %>%
...
in_average = data %>%
  ...
  mutate(
    remove = case_when(
      ...
      TRUE ~ "Keep"))
```
```{r 1-cleaning-21, fig.cap = "Average response to inattention check items"}
in_average %>%
...
```{r 1-cleaning-27, fig.cap = "Distribution of average time to respond to personality items."}
timing_data %>%
...
```{r 1-cleaning-33 }
data_path_2B = here("data/Wording 2B_August 4, 2021_18.49.text.csv")
...
```{r 1-cleaning-34}
names(data_2B) = str_replace(names(data_2B), "q763", "proid")
```
...
```{r 1-cleaning-43, echo = F}
olddata_2 = data_2
```

```{r 1-cleaning-44, echo = F}
data_2 = data_2 %>%
  mutate(proid = str_remove(proid, "Value will be set from panel or URL"),
         proid = str_remove(proid, "Value will be set from panel or UR"),
         proid = str_remove(proid, "TEST")) %>%
...
```{r 1-cleaning-47, ref.label='rm_proid_2'}
```
...
```{r 1-cleaning-51, fig.cap = "Maximum number of same consecutive responses in personality block 3."}
#visualize
runs_data_2 %>%
...
in_average = data_2 %>%
  ...
  mutate(
    remove = case_when(
      ...
      TRUE ~ "Keep"))
```
```{r 1-cleaning-55, fig.cap = "Average response to inattention check items"}
in_average %>%
  ggplot(aes(x = avg, fill = remove)) +
  geom_histogram(bins = 20, color = "white") +
  geom_vline(aes(xintercept = 4)) +
...
```{r 1-cleaning-61, fig.cap = "Distribution of average time to respond to personality items in Block 3."}
timing_data_2 %>%
  ggplot(aes(x = m_time, fill = remove)) +
  geom_histogram(color = "white") +
  labs(x = "Average response time (seconds)", y = "Number of participants") +
...
```{r 1-cleaning-64 }
data_2 = data_2 %>% 
  select(proid, very_delayed_recall, contains("_3")) %>% 
  mutate(time2 = "yes")
...
We also create a vector noting the items that are reverse scored. We use this later in tables, to help identify patterns when looking at analyses within-adjective.
...
```{r 1-cleaning-72, fig.cap = "Correct responses on the memory task"}
data %>%
  ggplot(aes(x = memory)) +
...
```{r 1-cleaning-75, fig.cap = "Distribution of delayed memory scores"}
data %>%
...
```{r 1-cleaning-76, fig.cap = "Relationship between immediate and delayed recall"}
data %>%
...
Finally, we score the memory challenge posed at Time 2. 
...
```{r 1-cleaning-78, fig.cap = "Distribution of delayed memory scores"}
data %>%
...
```{r 1-cleaning-79, fig.cap = "Relationship between immediate and delayed recall"}
data %>%
...
```{r 1-cleaning-90, fig.cap= "Distribution of seconds, raw and transformed."}
items_df %>% 
...
We replace the Prolific ID numbers with randomly generated numbers. This allows us to share the data without the risk of identifying participants or breaking their confidentiality.

```{r 1-cleaning-91}
set.seed(202108)
original_id = unique(data$proid)
new_id = stri_rand_strings(n = length(original_id), length = 10)

for(i in 1:length(original_id)){
  data$proid[data$proid == original_id[i]] = new_id[i]
  items_df$proid[items_df$proid == original_id[i]] = new_id[i]
}
```

```{r 1-cleaning-92, echo = F}
save(reverse, file = here("data/reverse_vector.Rds"))
