...
emmeans(mod.timing, ~"devicetype") %>% 
  as.data.frame() %>% 
  ggplot(aes(x = devicetype, y = emmean)) +
...
emmeans(mod.timing2, ~"devicetype*format") %>% 
  as.data.frame() %>% 
  ggplot(aes(x = format, y = emmean, group = devicetype)) +
...
emmeans(mod.responseD, ~"devicetype") %>% 
  as.data.frame() %>% 
  ggplot(aes(x = devicetype, y = emmean)) +
  geom_bar(stat = "identity", fill = "grey") + 
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = .5) +
...
emmeans(mod.responseD2, ~"devicetype*format") %>% 
  as.data.frame() %>% 
  ggplot(aes(x = format, y = emmean, group = devicetype)) +
  geom_bar(stat = "identity", aes(fill = devicetype),
           position = position_dodge(.9)) + 
