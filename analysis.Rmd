...
```{r}
data_labels = read_csv(here("data/Wording_July+11,+2021_17.17.csv"))

data = read_csv(here("data/Wording_July+11,+2021_17.17.csv"),
                skip = 3, col_names = names(data_labels))
rm(data_labels)
...
```{r}
contains_accurate = apply(data, 2, FUN = function(x) str_detect(x, "accurate"))
contains_accurate = apply(contains_accurate, 2, sum, na.rm=T)
contains_accurate = names(data)[contains_accurate > 0]
personality_items = select(data, proid, all_of(contains_accurate))
```
...
```{r}
data = select(data, -all_of(contains_accurate))
