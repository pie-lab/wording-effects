...
When examining only Block 1 data, item format was `r ifelse(fb1_sig, "associated", "unassociated")` with participants’ responses to personality items $(F(`r fb1_aov$NumDF[[1]]`, `r fb1_aov$DenDF[[1]]`) = `r fb1_aov$Fval[[1]]`, p = `r fb1_aov$Pval[[1]]`)$. See Figure \@ref(fig:analysis-format-7) for a visualization of this effect. In addition, Figure \@ref(fig:analysis-format-8) shows the full distribution of responses across format.
...
```{r analysis-format-7, fig.cap = "Predicted response on personality items by condition, using only Block 1 data.", echo = F}
...
```{r analysis-format-8, fig.cap = "Distribution of responses by category, block 1 data only", echo = F}
...
We apply a Holm correction to the _p_-values extracted from these analyses, to adjust for the number of tests conducted. We present results in Table \@ref(tab:mod1_item), which is organized by whether items were reverse-coded prior to analysis.
...
The pairwise comparisons of responses to different forms of "talkative" are displayed in Table \@ref(tab:analysis-format-11) and Figure \@ref(fig:analysis-format-12).

```{r analysis-format-11, results = 'asis', echo = F}
...
```{r analysis-format-12, fig.cap = "Average response to \"talkative\" by format (block 1 data only)", echo = F}
...
When examining both Block 1 and Block 2 data, item format was `r ifelse(fb2_sig, "associated", "unassociated")` with participants’ responses to personality items $(F(`r fb2_aov$NumDF[[1]]`, `r fb2_aov$DenDF[[1]]`) = `r fb2_aov$Fval[[1]]`, p = `r fb2_aov$Pval[[1]]`)$. You can see the effect visualized in Figure \@ref(fig:analysis-format-16). In addition, the full histogram of responses to each format are presented in Figure \@ref(fig:analysis-format-17).
...
```{r analysis-format-16, fig.cap = "Predicted response on personality items by condition, using Block 1 and Block 2.", echo =F}
...
```{r analysis-format-17, fig.cap = "Distribution of responses by category, block 1 and block 2", echo = F}
...
The pairwise comparisons of responses to different forms of "careless" are displayed in Table \@ref(tab:analysis-format-21) and Figure \@ref(fig:analysis-format-22).

```{r analysis-format-21, echo = F, results = 'asis'}
...
```{r analysis-format-22, echo = F, fig.cap = "Average response to \"careless\" by format (Block 1 and Block 2)" }
...
The pairwise comparisons of responses to different forms of "thrifty" are displayed in Table \@ref(tab:analysis-format-23) and Figure \@ref(fig:analysis-format-24).


```{r analysis-format-23, results = 'asis', echo = F}
...
```{r analysis-format-24, fig.cap = "Average response to \"thrifty\" by format (Block 1 and Block 2)", echo = F}
...
When examining both Block 1 and Block 2 data, memory `r ifelse(fmem_sig1, "had", "did not have")` a main effect on participant responses $(F(`r fmem_aov$NumDF[[2]]`, `r fmem_aov$DenDF[[2]]`) = `r fmem_aov$Fval[[2]]`, p = `r fmem_aov$Pval[[2]]`)$ and `r ifelse(fmem_sig2, "did", "did not")` moderate differences between formats $(F(`r fmem_aov$NumDF[[3]]`, `r fmem_aov$DenDF[[3]]`) = `r fmem_aov$Fval[[3]]`, p = `r fmem_aov$Pval[[3]]`)$. See these results in Figure \@ref(fig:analysis-format-26). 
...
Again, we test this model within each trait adjective, to determine whether the moderating effect of memory is stronger for any particular trait(s). The results are summarized in Table \@ref(tab:analysis-format-27).

```{r analysis-format-27, results = 'asis', echo = F}
...
```{r analysis-format-32, fig.cap = "Predicted response on personality items by condition, using only Block 1 data.", echo = F}
...
As before, we test both the additive (Table \@ref(tab:analysis-format-34b)) and interaction (Table \@ref(tab:analysis-format-35b)) terms of `format` and `i` for each item. 
...
mod_by_item_i = items_13 %>%
  ...
  mutate(aov = map(mod, anova))
```
```{r analysis-format-34b, results = 'asis', echo = F}
...
mod_by_item_i2 = items_13 %>%
  ...
  mutate(aov = map(mod, anova))
```
```{r analysis-format-35b, results = 'asis', echo = F}
...
The pairwise comparisons of responses to different forms of "curious" are displayed in Table \@ref(tab:analysis-format-37b) and Figure \@ref(fig:analysis-format-38).
...
curious_model_i = items_13 %>%
  ...
  lmer(response~format*i + (1|proid),
                               data = .)
```
```{r analysis-format-37b, results = 'asis', echo = F}
curious_model_i %>%
  tidy() %>%
...
The pairwise comparisons of responses to different forms of "lively" are displayed in Table \@ref(tab:analysis-format-39b) and Figure \@ref(fig:analysis-format-40).
...
```{r analysis-format-39b, results = 'asis', echo = F}
lively_model_i %>%
  tidy() %>%
  mutate(term = str_replace(term, "\n", " "),
         term = str_replace(term, "format", ""),
...
The pairwise comparisons of responses to different forms of "responsible" are displayed in Table \@ref(tab:analysis-format-41b) and Figure \@ref(fig:analysis-format-42).
...
```{r analysis-format-41b, results = 'asis', echo = F}
responsible_model_i %>%
  tidy() %>%
  mutate(term = str_replace(term, "\n", " "),
         term = str_replace(term, "format", ""),
...
The pairwise comparisons of responses to different forms of "sympathetic" are displayed in Table \@ref(tab:analysis-format-43b) and Figure \@ref(fig:analysis-format-44).
...
sympathetic_model_i = items_13 %>%
  ...
  lmer(response~format*i + (1|proid),
                               data = .)
```
```{r analysis-format-43b, results = 'asis', echo = F}
