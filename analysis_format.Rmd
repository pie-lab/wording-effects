...
item_responses = data %>%
  select(proid, all_of(items_seen_b1b2), delayed_memory)
```
...
item_responses = item_responses %>%
  gather(item, response, -proid, -delayed_memory) %>%
  mutate(
...
```{r}
item_block1 = filter(item_responses, block == "1")
...
```{r, results = 'asis'}
human_model_b1 = item_responses %>%
  filter(item == "human") %>%
  lm(response~format, data = .)

human_em_b1 = emmeans(human_model_b1, "format")
pairs(human_em_b1, adjust = "holm") %>%
  kable(booktabs = T,
        digits = c(0,2,2,2,2,3),
        caption = "Differences in response to Human by format (Block 1 data only)",
        col.names = c("Contrast", "Difference in means", "SE", "df", "t", "p")) %>%
  kable_styling()
```

```{r, fig.cap = "Average response to \"human\" by format (block 1 data only)" }
plot_model(human_model_b1, type = "pred", terms = c("format"))
```
...
```{r, results = 'asis'}
careless_model_b2 = item_responses %>%
...
```{r, results = 'asis'}
thrifty_model_b2 = item_responses %>%
  filter(item == "thrifty") %>%
  lmer(response~format + (1|proid), 
                               data = .)
...
```{r, results = 'asis'}
mod_by_item_mem = item_responses %>%
  group_by(item) %>%
  nest() %>%
