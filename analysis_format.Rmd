...
summary_by_item_b1 %>%
  kable(digits = c(0,2,2,2,3,3), booktabs = T, caption = "Format effects on response by item (block 1 data only)") %>%
  kable_styling()
```
...
We used a multilevel model, nesting response within participant to account for dependence. Our primary predictor was format. Here, we use data from blocks 1 and 2.
...
summary_by_item_b2 %>%
  kable(digits = c(0,2,2,0,2,2,3,3), booktabs = T, caption = "Format effects on response by item (block 1 data only)") %>%
  kable_styling()
```
...
summary_by_item_mem %>%
  kable(digits = c(0,0,2,2,2,3,3), booktabs = T) %>%
  kable_styling()
```
...
```{r}
mod.format_b3 = lmer(response~format*i + (1|proid), 
                  data = items_13)
anova(mod.format_b3)
```
...
```{r, fig.cap = "Predicted response on personality items by condition, using only Block 1 data."}
plot_b3 = plot_model(mod.format_b3, type = "pred", terms = c("format", "i")) 
plot_b3 +
  geom_line() +
  labs(x = NULL,
       y = "Average response",
