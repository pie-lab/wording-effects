...
Finally, we standardize responses within each block -- this allows us to use a regression framework yet interpret the slopes as correlations. 
...
tr_mod1_b2b3 = lmer(block_3 ~ block_2 + (1 |proid), data = items_matchb1)

tab_model(tr_mod1_b1b2, tr_mod1_b1b3, tr_mod1_b2b3, show.re.var = F)
```
...
tr_mod2_b1b3 = lmer(block_3 ~ block_1*condition + (1 |proid),
                    data = items_matchb1)
tr_mod2_b2b3 = lmer(block_3 ~ block_2*condition + (1 |proid),
                    data = items_matchb1)

tab_model(tr_mod2_b1b2, tr_mod2_b1b3, tr_mod2_b2b3, show.re.var = F)
```
...
```{r}
emtrends(tr_mod2_b2b3, pairwise ~ condition, var = "block_2")
```
