...
```{r, echo = F, eval = F}
# a different idea
item_block1 = filter(items_df, block %in% c("1", "2")) %>% 
  with_groups(item,
              ~mutate(., avg_response = mean(response))) %>% 
  with_groups(c(proid, item),
              ~mutate(., response = response - mean(response) + avg_response))

# add + (1|item) to analyses
