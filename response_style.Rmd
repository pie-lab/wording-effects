...
```{r responsestyle5, echo = F, cache = TRUE}
...
pred.expected = predictions(mod.expected, 
                            by = "format", 
                            type = "response")

plot_expected = pred.expected %>% 
  tibble() %>%
  mutate(format = case_when(format == "Adjective\nOnly" ~ "Adjective\nOnly",
                            format == "Am\nAdjective" ~ "\"Am\" +\nAdjective",
                            format == "Tend to be\nAdjective" ~ "\"Tend to be\" +\nAdjective",
                            format == "Am someone\nwho tends to be\nAdjective" ~ "\"Am someone who\ntends to be\" +\nAdjective")) %>%
  mutate(format = factor(format, levels = format)) %>%
  ggplot(aes(x = format, y = estimate, color = format)) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, lwd = 1.05) +
  geom_point(aes(color = NULL), stat = "identity", size = 3.5, color = "black") +
  geom_point(stat = "identity", size = 3) +
  labs(title = "Expected response",
       x = NULL, 
       y = NULL) + 
  coord_cartesian(ylim = c(4.1, 4.7)) +
  scale_color_manual(values = c("#5293BB", "#3776A1", "#185886", "#003A6B")) +
  scale_y_continuous(breaks = seq(4.10, 4.70, by = .10),
                     labels = sprintf("%.2f",seq(4.10, 4.70, by = .10))) +
...
```{r responsestyle9, results = 'asis', cache = TRUE}
...
pred.extreme = predictions(mod.extreme, 
                           by = "format", 
                           type = "response")

plot_extreme = pred.extreme %>% 
  tibble() %>%
  mutate(format = case_when(format == "Adjective\nOnly" ~ "Adjective\nOnly",
                            format == "Am\nAdjective" ~ "\"Am\" +\nAdjective",
                            format == "Tend to be\nAdjective" ~ "\"Tend to be\" +\nAdjective",
  ...
  scale_y_continuous(breaks = seq(.15, .25, by = .02),
                     labels = sprintf("%.2f", seq(.15, .25, by = .02))) +
  # coord_cartesian(ylim = c(.22, .32)) +
  # scale_y_continuous(breaks = seq(.22, .32, by = .02),
  #                    labels = sprintf("%.2f", seq(.22, .32, by = .02))) +
  scale_color_manual(values = c("#5293BB", "#3776A1", "#185886", "#003A6B")) +
  theme_pubclean(base_size = 15) +
  theme(legend.position = "none",
        title = element_text(face = "bold", size = 12))
...
```{r responsestyle18, results = 'asis', cache = TRUE}
...
```{r responsestyle28, fig.cap = "Likelihood of acquiescent responding to personality items by condition.", echo = F, cache = TRUE}
pred.yea = predictions(mod.yeasaying, 
                       by = "format", 
                       type = "response")
...
plot_yea = pred.yea %>% 
  ...
  scale_color_manual(values = c("#5293BB", "#3776A1", "#185886", "#003A6B")) +
  scale_y_continuous(breaks = seq(.55, .80, by = .05),
                     labels = sprintf("%.2f",seq(.55, .80, by = .05))) +
  theme_pubclean(base_size = 15) +
  theme(legend.position = "none",
        title = element_text(face = "bold", size = 12))
...
ggarrange(
  ...
  align = "hv",
  ncol = 1)
ggsave("response_style.png", height = 8, width = 8)
```
...
items_13$format = as.character(items_13$format)
```
