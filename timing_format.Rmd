...
logplot = plot_b1 %>% 
  ...
  scale_y_continuous(breaks = seq(.5, .9, by = .1),
                     labels = sprintf("%.2f",seq(.5, .9, by = .1))) +
...
rawplot = plot_b1 %>% 
  ...
  mutate(format = case_when(format == "Adjective\nOnly" ~ "Adjective\nOnly",
                            ...
                            format == "Am someone\nwho tends to be\nAdjective" ~ "\"See myself as someone\nwho tends to be\" +\nAdjective")) %>%
  mutate(format = factor(format, levels = format)) %>%
  ggplot(aes(x = format, y = predicted, color = format)) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, lwd = 1.05) +
  geom_point(aes(color = NULL), stat = "identity", size = 3.5, color = "black") +
  geom_point(stat = "identity", size = 3) +
  coord_cartesian(ylim = c(1.6, 2.4)) +
  scale_y_continuous(breaks = seq(1.6, 2.4, by = .2),
                     labels = sprintf("%.2f",seq(1.6, 2.4, by = .2))) +
  scale_color_manual(values = c("#5293BB", "#3776A1", "#185886", "#003A6B")) +
...
ggsave("timing_format.png", height = 4.5, width = 9)
```
